...
describe("/healthRecord", () => {
    ...
    describe('after login', () => {
        ...
        let token0;
        let premiumUserToken; //do authorization on this one later
        let decryptedToken0;
        let decryptedPremiumToken;
        beforeEach(async () => {
            ...
            await User.updateOne({ email: user1.email }, { $push: { roles: 'premiumUser'} });   //TODO add isPremiumUser code here.
            const res1 = await request(server).post("/login").send(user1);
            premiumUserToken = res1.body.token;
        ...
        describe("GET /", () => {
            ...
            it('should send 200 to premier user and return all healthRecords', async () => {
                const res = await request(server)
            ...
            it('should get status 403 for normal user', async () => {
                const res = await request(server)
                    .get("/healthRecord/all")
                    .set('Authorization', 'Bearer ' + token0)
                    .send();
